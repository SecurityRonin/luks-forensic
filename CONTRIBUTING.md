# Contributing to luks-forensic

Thanks for your interest in improving `luks-forensic`. These crates parse
untrusted LUKS containers acquired from potentially compromised systems, so
correctness and robustness are not negotiable. The bar is high and the workflow
is strict — please read this before opening a pull request.

The LUKS reader and analyzer ship on crates.io; see
...
against **real external data** where it matters: cross-check unlocking and
decryption against real LUKS containers and an independent oracle (`cryptsetup`),
...
- **Coverage** — 100% line coverage is enforced; no source line may be left
  ...
  happy path. A genuinely-unreachable defensive arm is annotated
  `// cov:unreachable: <invariant>` rather than deleted.
- **Fuzzing** — every parsed structure has a `cargo-fuzz` target whose invariant
  is "must not panic," plus a CI workflow that smoke-runs each target. A parser
  change must keep its fuzz target green.

## Robustness expectations

- No panics on malicious input — validate every length and offset against both
  the declared size and the actual buffer; read integers through the
  bounds-checked helpers; use checked or saturating arithmetic.
- Fail loud — surface malformed input as a typed error with enough context to
  diagnose it (the offending value verbatim). Never swallow an error or
  substitute a silent default.
- Never hand-roll a cryptographic primitive — use the audited RustCrypto crates.
